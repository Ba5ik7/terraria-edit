...
export class WorldParserService extends FileParserService {
  ...
  async parse(param1, param2) {
    ...
    if (typeof param1 == 'object') {
        ...
        if (param2) this.callback = param2;
      ...
      try {
          for (const [i, section] of allSections.entries()) {
              ...
              if (this.offset.value != this.world.pointers[i])
                  throw Error(section + ' section position did not end where it should');
          }
      } catch (e) {
        throw new Error('Problem with parsing the file' + e);
      }
  ...
  parseFileFormatHeader()
  {
      let data = {
        ...
        revision:       this.readUInt32(),
        favorite:       this.readBoolean(),
        pointers:       [],
        importants:     [],
      };
      this.skipBytes(7);
  ...
  parseHeader()
  {
      ...
      data.mapName                = this.readString(0);
      data.seedText               = this.readString(0);
      ...
      for (let i = this.readInt32(); i > 0; --i) {
          data.anglerWhoFinishedToday.push(this.readString(0));
      }
  ...
  parseChests()
  {
      ...
      for (let i = 0; i < data.chestsCount; i++) {
          ...
          data.chests[i].name = this.readString(0);
  ...
  parseSigns()
  {
      ...
      for (let i = 0; i < data.signsCount; i++) {
          data.signs[i] = {};
          data.signs[i].text = this.readString(0);
